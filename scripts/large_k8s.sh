...
__deployment_file="$__dir/config/deployment.yml"
__deployment_name="elasticsearch-deployment"
...
while ! [ "$(kubectl get po --no-headers | grep $__deployment_name | grep Running | awk '{print $2}')" = "2/2" ]; do
